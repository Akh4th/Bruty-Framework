...
# Modified password !
    ...
    if has_start.lower() == "yes":
        ...
        print(good + " ends with " + colored("[abc (1) / 4bc (2) / Abc (3) / 4Bc (4) / 4Bc< (5) / 123 (6)]", "red"))
        extra = input(colored("CUSTOMIZED PASSWORD (*)", "red"))
        while extra not in choices:
        ...
        if extra == "5":
            ...
            extra = nums
        # else:
        #     print(good + colored(" [abc = !] , [ABC = @] , [123 = #] , [!@# = $]", "green"))
        #     seq = input(colored("Sequence : ", "red"))
        #     while seq not in choices1 or len(seq) < max_length:
        #         seq = input(err + " Wrong input, try again : ")
        #     i = 0
        #     for i in list(seq):
        #         word = []
        #         if i == "!":
